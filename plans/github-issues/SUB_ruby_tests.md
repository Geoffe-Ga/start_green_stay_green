...
Sub-issue of the **Ruby (general)** epic. No code lands
without tests, and repo coverage must stay ≥90%. New generator branches for
Ruby need parallel coverage.

**Depends on:** Foundation, Quality sub-issues.

## Goal

Prove the tool generates correct, green Ruby (general)
projects, keeping repo coverage ≥90%.

## Scope — files to touch

- `tests/unit/generators/` — unit tests for each new/changed generator branch
- `tests/integration/generators/` — `green init -l ruby` generation assertions
- `tests/unit/test_multi_language.py` — add `ruby` to the parametrized
  all-language matrix
- `tests/e2e/` — generate a ruby project and assert it passes its own
...
- [ ] Parametrize `test_multi_language.py` to include `ruby`
...
- [ ] `ruby` appears in the parametrized multi-language tests
