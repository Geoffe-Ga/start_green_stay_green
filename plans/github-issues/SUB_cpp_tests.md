...
Sub-issue of the **C / C++ (Tizen (Samsung Galaxy Watch, native))** epic. No code lands
without tests, and repo coverage must stay ≥90%. New generator branches for
C / C++ need parallel coverage.

**Depends on:** Foundation, Quality, CI sub-issues.

## Goal

Prove the tool generates correct, green C / C++ (Tizen (Samsung Galaxy Watch, native))
projects, keeping repo coverage ≥90%.

## Scope — files to touch

- `tests/unit/generators/` — unit tests for each new/changed generator branch
- `tests/integration/generators/` — `green init -l cpp` generation assertions
- `tests/unit/test_multi_language.py` — add `cpp` to the parametrized
  all-language matrix
- `tests/e2e/` — generate a cpp project and assert it passes its own
...
- [ ] Parametrize `test_multi_language.py` to include `cpp`
...
- [ ] `cpp` appears in the parametrized multi-language tests
