...
Sub-issue of the **C# (general .NET)** epic. No code lands
without tests, and repo coverage must stay ≥90%. New generator branches for
C# need parallel coverage.

**Depends on:** Foundation, Quality sub-issues.

## Goal

Prove the tool generates correct, green C# (general .NET)
projects, keeping repo coverage ≥90%.

## Scope — files to touch

- `tests/unit/generators/` — unit tests for each new/changed generator branch
- `tests/integration/generators/` — `green init -l csharp` generation assertions
- `tests/unit/test_multi_language.py` — add `csharp` to the parametrized
  all-language matrix
- `tests/e2e/` — generate a csharp project and assert it passes its own
...
- [ ] Parametrize `test_multi_language.py` to include `csharp`
...
- [ ] `csharp` appears in the parametrized multi-language tests
