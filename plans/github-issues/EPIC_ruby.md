...
project regardless of language. This epic covers **Ruby**
(general).

**Current state:** Ruby is registered with `ci.py`/`dependencies.py`/`structure.py`/`readme.py`/`tests_gen.py` entries, but is **missing from `precommit.py`, `scripts.py`, `metrics.py`, and `architecture.py`** — pre-commit generation raises for Ruby.

No smartwatch platform; closes parity gaps so Ruby matches python/typescript as a fully green target.

## Goal

Complete production-grade **Ruby** support so
`green init -l ruby` generates a project that Starts Green and Stays Green
...
| Linters | RuboCop |
| Formatters | RuboCop (--autocorrect) |
...
Only the generator cells Ruby is actually missing are filed:
...
- [ ] `green init -l ruby` produces a project passing
      `pre-commit run --all-files` out of the box
- [ ] `green init -l python -l ruby` (multi-language) works via the
      YAML-aware pre-commit merge
- [ ] Parity matrix row for Ruby is fully ✅
- [ ] CLAUDE.md / README list Ruby as fully supported
