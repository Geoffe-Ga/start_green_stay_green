...
class TestDashboardSync:
    ...
    This test reads docs/dashboard.html from the repository root and compares
    it against the template generated by MetricsGenerator. It requires a full
    repository checkout (not suitable for unit tests).
    """

    # All element IDs that the dashboard JS reads/writes
    ...
    def _get_template(self) -> str:
        ...
        Returns:
            Dashboard HTML string from the template generator.
        """
        config = MetricsGenerationConfig(
            language="python",
    ...
    def _get_deployed(self) -> str:
        """Read the deployed docs/dashboard.html.

        Returns:
            Contents of docs/dashboard.html from the repo root.
        """
    ...
    def test_deployed_card_count_matches_template(self) -> None:
        """Verify docs/dashboard.html has the same card count as template."""
        ...
        assert deployed_cards == template_cards, (
            f"docs/dashboard.html has {deployed_cards} cards but template "
            f"has {template_cards}. "
            "Run: python scripts/regenerate_dashboard.py"
        )

    def test_deployed_has_all_value_ids(self) -> None:
        """Verify docs/dashboard.html has all expected value element IDs."""
        deployed = self._get_deployed()

        for element_id in self.EXPECTED_VALUE_IDS:
            expected = f'id="{element_id}"'
            assert expected in deployed, (
                f"docs/dashboard.html missing {expected}. "
                "Run: python scripts/regenerate_dashboard.py"
            )

    def test_deployed_has_all_status_ids(self) -> None:
        """Verify docs/dashboard.html has all expected status element IDs."""
        deployed = self._get_deployed()

        for element_id in self.EXPECTED_STATUS_IDS:
            expected = f'id="{element_id}"'
            assert expected in deployed, (
                f"docs/dashboard.html missing {expected}. "
                "Run: python scripts/regenerate_dashboard.py"
    ...
    def test_deployed_js_handles_all_metric_keys(self) -> None:
        """Verify the JS in docs/dashboard.html handles all metrics.json keys."""
        deployed = self._get_deployed()
