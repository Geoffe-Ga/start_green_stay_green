...
class _LanguageTooling:
    ...
    Attributes:
        ...
        docs_url: URL to the tool's documentation, or a local
            pointer when the tool is a generated, self-documenting
            artifact (the C/C++ checker).
...
_LANGUAGE_TOOLING: dict[str, _LanguageTooling] = {
    ...
    "cpp": _LanguageTooling(
        ...
        # The checker is generated and self-documenting (no external
        # tool docs exist), so the durable pointer is the artifact
        # itself rather than any remote URL that could go stale.
...
def _project_root() -> Path:
    ...
    assumed to be the project root — symlinks or exec-style invocation
    from another directory will resolve the wrong root, so run it from
    the project root in those setups.
    """
